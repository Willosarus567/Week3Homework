--DROP DATABASE Roon_Skaep_Taverns;
--Be sure to Insert the tables, then the data, then the key constraints
...
CREATE TABLE Payments_Recieved(
    ...
    Tavern_id INT,
    Guest_Id INT,
    PRIMARY KEY(Payment_Recieved_Id, Amount_Recieved),
...
INSERT INTO SERVICES(Service_Id, Service_Name, Status_Id)
            ...
            VALUES(1, 'Food & Beverage', 1),
                  (2, 'Equipment Smithing & Crafting', 1),
                  (3, 'Room Rentals', 1),
                  (4, 'Quest/Job Board', 1),
                  (5, 'Wares', 1),
                  (6, 'Rune Crafting', 2),
...
INSERT INTO ServiceStatuses(Status_Id, Service_Name, Status_Name)
            ...
            VALUES(2, 'Rune Crafting', 'InActive'),
                  (2, 'Team Rostering', 'InActive'),
                  (2, 'Quick Travel', 'InActive'),
                  (1, 'Food & Beverage', 'Active'),
                  (1, 'Equipment Smithing & Crafting', 'Active'),
                  (1, 'Room Rentals', 'Active'),
                  (1, 'Quest/Job Board', 'Active'),
                  (1, 'Wares', 'Active');
...
8. SELECT Guests.Level_Id FROM Guests WHERE Guests.Level_Id >= 1 AND Guests.Level_Id <= 40 
    UNION 
    SELECT Guests.Level_Id FROM Guests WHERE Guests.Level_Id >= 41 AND Guests.Level_Id <= 60
    UNION
    SELECT Guests.Level_Id FROM Guests WHERE Guests.Level_Id >= 61 AND Guests.Level_Id <= 126
    JOIN Classes ON Guests.Guest_Id = Classes.Guest_Id;-- I have tried very hard with the JOIN statement,
                                                        it has failed 

9. INSERT INTO payments_recieved (Payments_Recieved_Id, Date_Recieved, Amount_Recieved, Service_Name, Service_Id, Tavern_Id, Guest_Id) SELECT Tavern_Id, NULL, Guest_Name, NULL, NULL, Room_Id, Guest_Id FROM Rooms;
    --This too has failed. 
*/